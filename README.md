...
5. Для каждой части порождает еще более обогащенный XML **и** JSON-файл. XML выглядит почти так же, как в примерах выше, однако не содержит в себе информации о структуре документа: только набор предложений (`<se> ... </se>`) и слов (`<w> ... </w>`). JSON содержит иноформацию о структуре документа и о его атрибутах, а также **текст части целиком** как список предложений, каждое из которых является списком слов (просто сериализованные питоновские словари и списки). Это называется **прямой индекс**.
...
Также в этом проекте есть код сервера, отвечающего на поисковые запросы пользователей, его можно найти в `server.py`.

Для каждого запроса он парсит URL-параметры, формирует запрос для Яндекс.Сервера, получает ответ. Ответ приходит в виде:

1. Списка **групп**, которые соответствуют **документам** корпуса; элементы группы -- **части** документов. Каждая часть -- просто питоновский словарь, в котором можно найти прямой индекс и иерархию (струкруту) документа, полученную на этапе индексации (содержание JSON-файла).
2. Списка хитов (мест в прямом индексе, которые соответствуют найденным словам).

Дальше проиходит следующее.

Сервер начинает полный обход дерева, описывающего сруктуру документа, и использует информацию о том, какие предложения покрывает тот или иной терминальный узел, чтобы наполнить эти самые терминальные узлы словами из прямого индекса. Параллельно он формирует сниппеты (по довольно сложной логике, см. `processor.py`), добавляя в иерархию элементы типа `<corpus_type>:snippet`. Сниппеты нужны для того, чтобы показывать некоторую окрестность вокруг хита (а не весь документ). Итоговая дополненная и "восстановленная" словами структура документа отдается на рендер.

Рендер работает так. На этапе индексации каждый элемент структуры документа получал свой собственный **тип** (например, `accent:speach`), который основан на имени соответствующего тэга в исходном XML и модифицирован типом корпуса.
Сервер запросов умеет соотносить тип элемента структуры документа с типом `writer`'a (`render/writers.py`), который нужно использовать, чтобы отображать результаты поиска. Выглядит это примерно вот так (`search.py`):
...
`writer`'ы проходят по дереву структуры документа и формируют XSLT-документ, который потом рендерится в HTML (см. `render/xslt.py`), и этот итоговый HTML отдается пользователю.
...
clear && python index.py --index --dir /place/ruscorpora/texts/finalized/accent/accent_main/ --kps 10134 --corpus_type accent --subcorpus accent --nodisk > accent_main.log 2>&1
clear && python index.py --index --dir /place/ruscorpora/texts/finalized/accent/accent_stihi/ --kps 10134 --corpus_type accent_stihi --subcorpus poetic --nodisk > accent_stihi.log 2>&1
clear && python index.py --index --dir /place/ruscorpora/texts/finalized/accent/accent_poetic/ --kps 10134 --corpus_type accent_poetic --subcorpus poetic --nodisk > accent_poetic.log 2>&1
...
clear && python index.py --index --dir /place/ruscorpora/texts/finalized/spoken/spoken/ --kps 10145 --corpus_type spoken_spoken --subcorpus spoken --nodisk > spoken_spoken.log 2>&1
clear && python index.py --index --dir /place/ruscorpora/texts/finalized/spoken/accent_main/ --kps 10145 --corpus_type spoken_accent --subcorpus accent --nodisk > spoken_accent.log 2>&1
...
cd /home/zavgorodny/saas/
rm -f murco* && clear && python index.py --index --dir /place/ruscorpora/texts/finalized/murco/ --kps 10152 --corpus_type murco --subcorpus murco --nodisk > murco.log 2>&1
```
...
cd /home/zavgorodny/saas/
rm -f paper* && clear && python index.py --index --dir /place/ruscorpora/texts/finalized/regional_grodno_rus/ --kps 10160 --subcorpus regional_rus --corpus_type regional_rus --nodisk > regional_rus.log 2>&1
```
